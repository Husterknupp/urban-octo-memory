...
class TC_activation_key {
  ...
  function __construct ( $args ) {
      ...
      $this -> strings = array(
        'theme-license' => __( 'Theme activation key', 'customizr-pro' ),
        'enter-key' => __( 'Enter your Activation Key and press "Save Changes"', 'customizr-pro' ),
        'license-key' => __( 'Activation Key', 'customizr-pro' ),
        'license-action' => __( 'Key Action', 'customizr-pro' ),
        'deactivate-license' => __( 'Deactivate Key', 'customizr-pro' ),
        'activate-license' => __( 'Activate Key', 'customizr-pro' ),
        'status-unknown' => __( 'Key status is unknown.', 'customizr-pro' ),
        'renew' => __( 'Renew now.', 'customizr-pro' ),
        'not-receiving-updates' => __( "You are not receiving updates for bug fixes and new features anymore.", 'customizr-pro' ),
        'unlimited' => __( 'unlimited', 'customizr-pro' ),
        'license-key-is-valid' => __( 'Your key is valid, you are safely receiving the latest theme updates for bug fixes and new features.', 'customizr-pro' ),
        'will_be_renewed%s' => __( 'Will be renewed %s.', 'customizr-pro' ),
        'expires-never'             => __( 'Lifetime Activation Key.', 'customizr-pro' ),
        '%1$s/%2$-sites' => __( 'You have %1$s / %2$s sites activated.', 'customizr-pro' ),
        'license-key-expired-%s' => __( 'Key expired %s.', 'customizr-pro' ),
        'license-key-expired' => __( 'Key has expired.', 'customizr-pro' ),
        'license-key-lifetime' => __( 'Lifetime duration.', 'customizr-pro' ),
        'license-keys-do-not-match' => __( 'Keys do not match.', 'customizr-pro' ),
        'license-is-inactive' => __( 'Activation key is inactive.', 'customizr-pro' ),
        'license-key-is-disabled' => __( 'Activation key is disabled.', 'customizr-pro' ),
        'site-is-inactive' => __( 'Site is inactive.', 'customizr-pro' ),
        'license-status-unknown' => __( 'Activation key status is unknown. This is usually due to a connection problem or a server firewall issue.', 'customizr-pro' ),
        'update-notice' => __( "Updating this theme will lose any customizations you have made. 'Cancel' to stop, 'OK' to update.", 'customizr-pro' ),
        'update-available' => __('<strong>%1$s %2$s</strong> is available. <a href="%3$s" class="thickbox" title="%4s">Check out what\'s new</a> or <a href="%5$s"%6$s>update now</a>.', 'customizr-pro' )
      );
    ...
    */
    function tc_licenses_menu() {
        add_theme_page(
          sprintf( __('%1$s Key', 'customizr-pro') , $this -> theme_name ),
          sprintf( __('%1$s Key', 'customizr-pro') , $this -> theme_name ),
          'manage_options',
          'tc-licenses',
      ...
      ob_start();
        ?>
          <div class="update-nag" style="position:relative;width: calc( 100% - 56px );">
            <h3><?php _e("You did not activate the Customizr Pro theme key yet.", 'customizr-pro') ?></h3>
            <p>
              ...
              <p style="text-align:right;position: absolute;right: 5px;bottom: -8px;">
                <?php printf('<em>%1$s <a href="#" title="%1$s" class="tc-dismiss-key-update"> ( %2$s <strong>x</strong> ) </a></strong></em>',
                    __("OK got it. Remind me later.", 'customizr-pro' ),
    ...
    function _theme_license_page_content($license, $status, $message) {
        ...
        <div class="activation-key-wrapper" style="background: #fff;padding: 1px 12px;margin: 5px 0 15px;">
            <form method="post" action="options.php">
                ...
                <h2>&nbsp;<?php printf( __('%1$s Key', 'customizr-pro') , $this -> theme_name ) ; ?></h2>
                ...
                <table class="form-table">
                    <tbody>
                        <tr valign="top">
                            ...
                            <td>
                                <input id="tc_<?php echo $this->theme_prefix ?>_license_key" name="tc_<?php echo $this->theme_prefix ?>_license_key" type="text" class="regular-text" value="<?php esc_attr_e( $license, 'customizr-pro' ); ?>" />
                            ...
                            <tr valign="top">
                                ...
                                <td>
                                  <?php if( $status !== false && 'valid' == $status )  : ?>
                                      <span style="color:green;line-height: 27px;font-weight: bold;"><?php _e('active', 'customizr-pro'); ?></span>
                                      <input type="submit" class="button-secondary" name="tc_<?php echo $this->theme_prefix ?>_license_desactivate" value="<?php esc_attr_e( $strings['deactivate-license'], 'customizr-pro' ); ?>"/>
                                  <?php else : ?>
                                      <input type="submit" class="button-secondary" name="tc_<?php echo $this->theme_prefix ?>_license_activate" value="<?php esc_attr_e( $strings['activate-license'], 'customizr-pro' ); ?>"/>
                                  <?php endif; ?>
                                </td>
                            </tr>
                        <?php } ?>
            ...
            <hr>
            <h3><span class="dashicons-before dashicons-info" style="color:#0085ba">&nbsp;</span><?php _e('Helpful links', 'customizr-pro'); ?></h3>
            <ul>
      ...
      ob_start();
        ?>
          <div class="update-nag">
            <h3><?php _e( "We couldn't reach our remote server to get the latest version of the theme.", 'customizr-pro') ?></h3>
            <p><?php _e( "This can happen sometimes when your host server filters requests to remote servers.", 'customizr-pro'); ?></p>
            <p><?php _e( "If the problem persists after several tries, please follow those simple steps to update your theme : ", 'customizr-pro' ); ?>
              <ol>
                ...
                <li><?php _e("Download the latest version of the theme", 'customizr-pro') ?></li>
                <li>
    ...
    *******************************************************/
    ...
    function tc_get_license_error_message( $license_data ) {
        ...
        switch( $_error ) {
            ...
            case 'expired' :
              ...
              $message = sprintf(
                __( 'Your activation key expired on %s.', 'customizr-pro' ),
            ...
            case 'revoked' :
              ...
              $message = __( 'Your activation key has been disabled.', 'customizr-pro' );
              break;

            case 'missing' :

              $message = __( 'Invalid activation key.', 'customizr-pro' );
              break;

            case 'invalid' :
            case 'site_inactive' :

              $message = __( 'Your activation key is not active for this URL.', 'customizr-pro' );
              break;

            case 'item_name_mismatch' :
              $message = sprintf( __( 'This appears to be an invalid activation key for %s.', 'customizr-pro' ), isset( $license_data->item_name ) ? str_replace('+', ' ', $license_data->item_name) : __('your product', 'customizr-pro') );
              break;

            case 'no_activations_left':

              $message = __( 'Your key has reached its activation limit.', 'customizr-pro' );
              break;

            default :
              $message =  __( "<u>An error occurred, please try again.</u> In most of the cases, it's due to a wrong key, like for example a missing character. Make sure you copy the correct activation key. Sometimes there can be a missing character at the beginning or at the end.", 'customizr-pro' );
              $message .= sprintf( '&nbsp;<a href="//docs.presscustomizr.com/article/10-where-can-i-find-my-activation-key" target="_blank">%1$s</a>', __('Where can you find your activation key ?', 'customizr-pro') );
              break;
          }
        return $message;
    }
    ...
    function _update_upgrade_transient($license_data) {
      ...
      if ( isset($license_data -> error ) && $license_data -> error  == 'no_activations_left' ) {
        ...
        ?>
          <div class="updated">
            <h3><?php _e("You've reached the limit of activated websites for your current plan.", 'customizr-pro') ?></h3>
            <?php
              printf( '<p>%1$s <strong><a href="%2$s" target="_blank">%3$s</a></strong></p>',
                    __( "Connect to your account on presscustomizr.com", 'customizr-pro' ),
                    esc_url('docs.presscustomizr.com/article/18-how-can-i-upgrade-my-current-package'),
                    __( "and upgrade your plan.", 'customizr-pro' )
              );
            ?>
          </div>
        <?php
    ...
    if ( is_wp_error( $response ) || 200 != wp_remote_retrieve_response_code( $response ) ) {
        ...
        if ( false === $license_data->success ) {
            ...
            if( $current_site_activation_status === false || $current_site_activation_status  != 'valid' ) {
                  $message = sprintf( '<span style="color:#f57717;line-height: 27px;font-weight: bold;">%1$s </span>', __('Key is not activated for this website yet. Enter your key and press "Activate Key".', 'customizr-pro') );
              } else {
                $message = sprintf( '<span style="color:green;line-height: 27px;font-weight: bold;">%1$s </span>', __('Key is activated for this website.', 'customizr-pro') );
            ...
            // Site is inactive
            $message = sprintf( __( 'This appears to be an invalid activation key for %s.', 'customizr-pro' ), isset( $license_data->item_name ) ? str_replace('+', ' ', $license_data->item_name) : __('your product', 'customizr-pro') );
            $message .= '&nbsp' . sprintf( __( 'This can for example happen if you try to use a Bundle key instead of the product key.', 'customizr-pro' ) );
            $message .= '&nbsp' . sprintf( __( '<a href="https://docs.presscustomizr.com/article/11-how-to-activate-your-product-key" target="_blank">%1$s</a>', 'customizr-pro' ), __('How to activate your product key ?', 'customizr-pro') );
        } else {
            $message = $strings['license-status-unknown'];
        }
    }
