    ...
    */
    function czr_fn_render_grid_control_link( $set_id ) {
      if ( false !== strpos( $set_id, 'tc_post_list_show_thumb' ) ) {
        printf('<span class="tc-grid-toggle-controls" title="%1$s">%1$s</span>' , __('More grid design options' , 'customizr-pro'));
      }
    }
    ...
    */
    function czr_fn_add_favicon_title($set_id) {
      if ( false !== strpos( $set_id, 'tc_sticky_logo_upload' ) )
        printf( '<h3 class="czr-customizr-title">%s</h3>', __( 'SITE ICON' , 'customizr-pro') );
    }
    ...
    */
    function czr_fn_add_fallback_page() {
        $theme_page = add_theme_page(
            __( 'Upgrade WP' , 'customizr-pro' ),   // Name of page
            __( 'Upgrade WP' , 'customizr-pro' ),   // Label in menu
    ...
    function czr_fn_fallback_admin_page() {
      ?>
        <div class="wrap upgrade_wordpress">
          ...
          <h2><?php _e( 'This theme requires WordPress 3.4+' , 'customizr-pro' ) ?> </h2>
          <br />
          <p style="text-align:center">
            <a style="padding: 8px" class="button-primary" href="<?php echo admin_url().'update-core.php' ?>" title="<?php _e( 'Upgrade Wordpress Now' , 'customizr-pro' ) ?>">
            <?php _e( 'Upgrade Wordpress Now' , 'customizr-pro' ) ?></a>
            <br /><br />
    ...
    function czr_fn_get_translated_strings() {
      return apply_filters('controls_translated_strings',
          array(
                ...
                'rss' => __('Rss', 'customizr-pro'),
                'selectSocialIcon' => __('Select a social icon', 'customizr-pro'),
                'followUs' => __('Follow us on', 'customizr-pro'),
                'successMessage' => __('Done !', 'customizr-pro'),
                'socialLinkAdded' => __('New Social Link created ! Scroll down to edit it.', 'customizr-pro'),
                'readDocumentation' => __('Learn more about this in the documentation', 'customizr-pro'),
                //WP TEXT EDITOR MODULE
                'textEditorOpen' => __('Edit', 'customizr-pro'),
                'textEditorClose' => __('Close Editor', 'customizr-pro'),
                //SLIDER MODULE
                'slideAdded'   => __('New Slide created ! Scroll down to edit it.', 'customizr-pro'),
                'slideTitle'   => __( 'Slide', 'customizr-pro'),
                'postSliderNote' => __( "This option generates a home page slider based on your last posts, starting from the most recent or the featured (sticky) post(s) if any.", "customizr-pro" ),
                'sidenavNote'  => sprintf( '%1$s<br/>%2$s',
                                    __( 'The side on which the menu is revealed depends on the choosen header layout.', 'customizr-pro'),
                                    sprintf( __("To change the global header layout, %s" , "customizr-pro"),
                                      sprintf( '<a href="%1$s" title="%3$s">%2$s &raquo;</a>',
                                        "javascript:wp.customize.section('header_layout_sec').focus();",
                                        __("jump to the Design and Layout section" , "customizr-pro"),
                                        __("Change the header layout", "customizr-pro")
                                      )
                                    )
                                  ),
              ...
              if ( CZR_THEME_OPTIONS.'[tc_front_slider]' == $this -> id ) {
                ...
                if ( empty( $sliders ) ) {
                  printf('<div class="czr-notice" style="padding: 5px;"><span class="czr-notice">%1$s<br/><a class="button-primary" href="%2$s" target="_blank">%3$s</a><br/><span class="tc-notice">%4$s <a href="%5$s" title="%6$s" target="_blank">%6$s</a></span></span>',
                    __("You haven't create any slider yet. Go to the media library, edit your images and add them to your sliders.", "customizr-pro" ),
                    admin_url( 'upload.php?mode=list' ),
                    __( 'Create a slider' , 'customizr-pro' ),
                    __( 'Need help to create a slider ?' , 'customizr-pro' ),
                    esc_url( "http://docs.presscustomizr.com/article/3-creating-a-slider-with-customizr-wordpress-theme" ),
                    __( 'Check the documentation' , 'customizr-pro' )
                  );
                }
              }
            break;
  ...
  class CZR_Customize_Upload_Control extends WP_Customize_Control {
    ...
    public function render_content() {
      ...
      <label>
        ...
        <?php endif; ?>
        <div>
          <a href="#" class="button-secondary czr-upload"><?php _e( 'Upload' , 'customizr-pro'  ); ?></a>
          <a href="#" class="remove"><?php _e( 'Remove' , 'customizr-pro'  ); ?></a>
        </div>
        <?php if(!empty( $this -> notice)) : ?>
          <span class="czr-notice"><?php echo $this -> notice; ?></span>
        <?php endif; ?>
    ...
    protected function czr_fn_render_template() {
      ...
      <li id="accordion-panel-{{ data.id }}" class="accordion-section control-section control-panel control-panel-{{ data.type }}">
        <h3 class="accordion-section-title" tabindex="0">
          ...
          <span class="screen-reader-text"><?php _e( 'Press return or enter to open this panel', 'customizr-pro' ); ?></span>
        </h3>
