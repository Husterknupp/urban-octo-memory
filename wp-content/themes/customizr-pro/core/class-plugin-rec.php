...
function czr_fn_maybe_render_rec_notice() {
  ...
  if ( $is_nimble_installed ) {
    ...
    $button_text = __( 'Activate Nimble Builder Now', 'customizr-pro' );
    $button_link = wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . $plugin . '&amp;plugin_status=all&amp;paged=1&amp;s', 'activate-plugin_' . $plugin );
  } else {
    if ( !current_user_can( 'install_plugins' ) ) {
      return;
    }
    $button_text = __( 'Install Nimble Builder Now', 'customizr-pro' );
...
function czr_fn_print_l_rec_notice( $button_text, $button_link ) {
  $heading = sprintf( __('Customizr theme recommends %1$s.', 'customizr-pro' ),
          sprintf('<a href="%1$s" class="thickbox" target="_blank">%2$s</a>',
              ...
              __('Nimble Page Builder', 'customizr-pro')
          )
      );

  $message = sprintf( '<span style="font-weight:normal;">%1$s<br/> %2$s<br/>%3$s<br/>%4$s</span>',
    __( 'Developers of the Customizr theme have created Nimble Builder, a free, powerful yet easy-to-use page builder already active on 50K+ WordPress websites.',  'customizr-pro'),
    __( 'It allows you to drag and drop mobile-ready sections on <i>really</i> any page of your site, including home, posts, pages, products, archives, 404, search pages, ...', 'customizr-pro' ),
    sprintf(
        __( 'You can insert simple text zones, but also create %1$s, insert post grids, column structures, buttons, widget zones, maps, icons, and much more, or use pre-designed sections with professional %2$s.', 'customizr-pro'),
        sprintf('<a href="%1$s" target="_blank" title="%2$s">%2$s</a>', esc_url('nimblebuilder.com/mp4-video-background-with-delay/'), __('video backgrounds', 'customizr-pro') ),
        sprintf('<a href="%1$s" target="_blank" title="%2$s">%2$s</a>', esc_url('demo.presscustomizr.com/nimble-builder/'), __('parallax effect', 'customizr-pro') )
    ),
    __( "The plugin is lightweight and has been designed to integrate seamlessly with Customizr and any WordPress theme.", 'customizr-pro')
  );
...
function czr_fn_print_s_rec_notice( $button_text, $button_link ) {
  $heading = sprintf( __('Customizr theme recommends the simple and smart %1$s companion ( free 😍 ) to help you build pages %2$s.', 'customizr-pro' ),
      sprintf('<a href="%1$s" class="thickbox" target="_blank">%2$s</a>',
          wp_nonce_url( 'plugin-install.php?tab=plugin-information&amp;plugin=nimble-builder&amp;TB_iframe=true&amp;width=640&amp;height=500'),
          __('Nimble Builder', 'customizr-pro')
      ),
      sprintf('<a href="https://nimblebuilder.com/landing-page-one/" target="_blank" rel="noreferrer noopener">%1$s</a>',
          __('like this', 'customizr-pro')
      )
  );
  $notice_id = REC_NOTICE_ID;
  ?>
