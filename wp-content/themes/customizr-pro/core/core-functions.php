...
function czr_fn_wp_title( $title, $sep ) {
    ...
    if ( $paged >= 2 || $page >= 2 )
      $title = "$title $sep " . sprintf( __( 'Page %s' , 'customizr-pro' ), max( $paged, $page ) );
...
function czr_fn_slider_choices() {
  ...
  $slider_choices = array(
    0     =>  __( '&mdash; No slider &mdash;' , 'customizr-pro' ),
    'demo'  =>  __( '&mdash; Demo Slider &mdash;' , 'customizr-pro' ),
...
function czr_fn_sanitize_email( $value) {
  ...
  if ( czr_fn_is_customizing() && $value && !$sanitized_value ) {
    return new WP_Error( 'required', __( 'Please fill the email input with a valid email address', 'customizr-pro' ) );
  }
