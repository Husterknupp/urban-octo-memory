...
function czr_fn_comment_info( $args = array() ) {
      ...
      $link            = sprintf( '%1$s<a class="comments__link" data-anchor-scroll="true" href="%2$s" title="%3$s" %5$s><span>%4$s</span></a>%6$s',
            $before,
            $link,
            sprintf( "%s %s %s" , number_format_i18n( $comments_number ) , _n( 'Comment on' , 'Comments on' , $comments_number, 'customizr-pro' ) ,  esc_attr( strip_tags( get_the_title() ) ) ),
            sprintf( "%s %s" , number_format_i18n( $comments_number ) , _n( 'comment' , 'comments' , $comments_number, 'customizr-pro' ) ),
            $link_attributes,
            $after
      );
...
function czr_fn_readmore_button( $args = array() ) {
      $defaults = array(
            'class'   => '',
            'link'    => get_permalink(),
            'text'    => __('Read more &raquo;', 'customizr-pro' ),
            'esc_url' => true,
            'echo'    => false,
      );
...
function czr_fn_edit_button( $args = array() ) {
      ...
      */
      ...
      $defaults = array(
        'class'     => '',
        'title'     => __( 'Edit', 'customizr-pro' ),
        'text'      => __( 'Edit', 'customizr-pro' ),
        'link'      => '#',
        'target'    => '',//'_blank',
        'rel'       => 'nofollow',
        'echo'      => true,
    ...
    if ( current_user_can( 'edit_theme_options' ) && ! czr_fn_is_there_any_visible_menu_location_assigned() ) {
        czr_fn_edit_button(
            array(
              ...
              'text'  => __( 'Add a menu', 'customizr-pro' ),
              'title' => __( 'open the customizer menu section', 'customizr-pro'),
