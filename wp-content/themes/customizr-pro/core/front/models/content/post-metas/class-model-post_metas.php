...
class CZR_post_metas_model_class extends CZR_Model {
  ...
  private function czr_fn_meta_generate_author( $before ) {
    ...
    $before = is_null($before) ? __( 'by', 'customizr-pro' ) : $before;
  ...
  private function czr_fn_meta_generate_pub_date( $format = '', $permalink = false, $before = null, $only_text = false ) {
    ...
    $before = is_null($before) ? __( 'Published', 'customizr-pro' ) : $before;
  ...
  private function czr_fn_meta_generate_up_date( $format = '', $permalink = false, $before = null, $only_text = false ) {
    ...
    $before = is_null($before) ? __( 'Updated', 'customizr-pro' ) : $before;
  ...
  public function czr_fn_meta_generate_attachment_image_info() {
    ...
    $_html_parts  = array(
      ( isset($metadata['width']) && isset($metadata['height']) ) ? '<span class="attachment-size">' . __('at dimensions' , 'customizr-pro').'<a href="'.esc_url( wp_get_attachment_url() ).'" title="'.__('Link to full-size image' , 'customizr-pro').'" target="_blank"> '.$metadata['width'].' &times; '.$metadata['height'].'</a></span>' : '',
      //when post parent id is 0 means that the media is not attached to any post
      ( 0 != $post->post_parent ) ? '<span class="attachment-parent">' . __('in' , 'customizr-pro') . '<a href="' . esc_url( get_permalink( $post->post_parent ) ) . '" title="' . the_title_attribute( array( 'before' => __('Return to ' , 'customizr-pro'), 'echo' =>false ) ) .'" rel="gallery"> '.strip_tags( get_the_title( $post->post_parent ) ).'</a></span>' : ''
    );
  ...
  protected function czr_fn_get_meta_date( $pub_or_update = 'publication', $_format = '', $permalink = false, $only_text = false ) {
    ...
    if ( ! $only_text ) {
        $date_meta = sprintf( '<a href="%1$s" title="%2$s" rel="bookmark">%3$s</a>',
            ...
            $permalink ? esc_attr( the_title_attribute( array( 'before' => __('Permalink to:&nbsp;', 'customizr-pro'), 'echo' => false ) ) ) : esc_attr( get_the_time() ),
            $date_meta
        );
    }
