...
function czr_fn_display_min_php_message() {
    czr_fn_display_min_requirement_notice( __( 'PHP', 'customizr-pro' ), CZR_MIN_PHP_VERSION );
}

function czr_fn_display_min_wp_message() {
    czr_fn_display_min_requirement_notice( __( 'WordPress', 'customizr-pro' ), CZR_MIN_WP_VERSION );
}
...
function czr_fn_display_min_requirement_notice( $requires_what, $requires_what_version ) {
    ...
    printf( '<div class="error"><p>%1$s</p></div>',
        sprintf( __( 'The <strong>%1$s</strong> theme requires at least %2$s version %3$s', 'customizr-pro' ),
            $theme,
            $requires_what,
