...
<section class="post-author <?php czr_fn_echo('element_class') ?>" <?php czr_fn_echo('element_attributes') ?>>
  <div class="post-author__wrapper">
    <h4 class="post-author-title"><?php echo _n( 'AUTHOR', 'AUTHORS', $authors_number, 'customizr-pro' ) ?></h4>
  <?php foreach ( $authors_id as $author_id ) :?>
    <figure class="author-info">
      ...
      <figcaption>
        ...
        <a href="<?php echo esc_url( get_author_posts_url( $author_id ) ) ?>" rel="author" class="action-link" title="<?php _e( 'View all the posts of the author', 'customizr-pro' ) ?>">
          <?php
            $author_posts = count_user_posts( $author_id );
            printf( _n('%s post', '%s posts', $author_posts , 'customizr-pro' ), $author_posts );
          ?>
        </a>
