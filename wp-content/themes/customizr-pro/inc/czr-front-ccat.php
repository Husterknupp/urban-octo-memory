    ...
    */
    function czr_fn_logo_img_view( $_args ){
      ...
      $_html = sprintf( '<img src="%1$s" alt="%2$s" %3$s %4$s %5$s %6$s class="%7$s %8$s"/>',
        $logo_src,
      	apply_filters( 'tc_logo_alt', __( 'Back Home' , 'customizr-pro' ) ),
    ...
    */
    function czr_fn_menu_button_view( $args ) {
      ...
      $_button_label = sprintf( '<span class="menu-label">%s</span>',
        '__sidenav' == current_filter() ? __('Close', 'customizr-pro') : __('Menu' , 'customizr-pro')
      );
      $_button = sprintf( '<div class="%1$s"><button type="button" class="btn menu-btn" %2$s title="%5$s" aria-label="%5$s">%3$s%3$s%3$s </button>%4$s</div>',
        ...
        '__sidenav' == current_filter() ? __('Close', 'customizr-pro') : __('Open the menu' , 'customizr-pro')
      );
    ...
    */
    ...
    function czr_fn_list_pages( $args = '' ) {
      $defaults = array(
        ...
        'child_of' => 0, 'exclude' => '',
        'title_li' => __( 'Pages', 'customizr-pro' ), 'echo' => 1,
      ...
      function czr_fn_404_content() {
          ...
          echo apply_filters( 'tc_404_content',
              sprintf('<div class="%1$s"><div class="entry-content"><p>%2$s</p> %3$s</div>%4$s</div>',
                  'tc-content span12',
                  __( 'Sorry, but the requested page is not found. You might try a search below.' , 'customizr-pro' ),
    ...
    class CZR_attachment {
        ...
        function czr_fn_attachment_content() {
            ...
            <nav id="image-navigation" class="navigation">
                <span class="previous-image"><?php previous_image_link( false, __( '&larr; Previous' , 'customizr-pro' ) ); ?></span>
                <span class="next-image"><?php next_image_link( false, __( 'Next &rarr;' , 'customizr-pro' ) ); ?></span>
...
class CZR_breadcrumb {
    ...
    function _get_args() {
    	$args =  array(
		  ...
		  'after'      => false,
		  'front_page' => true,
		  'show_home'  => __( 'Home' , 'customizr-pro' ),
		  'network'    => false,
		  'echo'       => false
	  	);
		...
		$defaults = array(
			'container'  => 'div' , // div, nav, p, etc.
			'separator'  => '/' ,
			'before'     => __( 'Browse:' , 'customizr-pro' ),
			'after'      => false,
			'front_page' => true,
			'show_home'  => __( 'Home' , 'customizr-pro' ),
			'network'    => false,
    ...
    */
	...
	function czr_fn_breadcrumb_trail_get_items( $args = array() ) {
		...
		elseif ( is_archive() ) {
			...
			elseif ( is_time() ) {
				...
				if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
					$trail[] = get_the_time( __( 'g:i a' , 'customizr-pro' ) );

				elseif ( get_query_var( 'minute' ) )
					$trail[] = sprintf( __( 'Minute %1$s' , 'customizr-pro' ), get_the_time( __( 'i' , 'customizr-pro' ) ) );

				elseif ( get_query_var( 'hour' ) )
					$trail[] = get_the_time( __( 'g a' , 'customizr-pro' ) );
			}
			...
			elseif ( is_date() ) {
				...
				if ( is_day() ) {
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y' , 'customizr-pro' ) ) . '">' . get_the_time( __( 'Y' , 'customizr-pro' ) ) . '</a>';
					$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F' , 'customizr-pro' ) ) . '">' . get_the_time( __( 'F' , 'customizr-pro' ) ) . '</a>';

					if ( is_paged() )
						$trail[] = '<a href="' . get_day_link( get_the_time( 'Y' ), get_the_time( 'm' ), get_the_time( 'd' ) ) . '" title="' . get_the_time( esc_attr__( 'd' , 'customizr-pro' ) ) . '">' . get_the_time( __( 'd' , 'customizr-pro' ) ) . '</a>';
					else
						$trail[] = get_the_time( __( 'd' , 'customizr-pro' ) );
				}
				...
				elseif ( get_query_var( 'w' ) ) {
					...
					if ( is_paged() )
						$trail[] = get_archives_link( add_query_arg( array( 'm' => get_the_time( 'Y' ), 'w' => get_the_time( 'W' ) ), esc_url(home_url()) ), sprintf( __( 'Week %1$s' , 'customizr-pro' ), get_the_time( esc_attr__( 'W' , 'customizr-pro' ) ) ), false );
					else
						$trail[] = sprintf( __( 'Week %1$s' , 'customizr-pro' ), get_the_time( esc_attr__( 'W' , 'customizr-pro' ) ) );
				}
				...
				elseif ( is_month() ) {
					...
					if ( is_paged() )
						...
						$trail[] = get_the_time( __( 'F' , 'customizr-pro' ) );
				}

				elseif ( is_year() ) {

					if ( is_paged() )
						$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . esc_attr( get_the_time( __( 'Y' , 'customizr-pro' ) ) ) . '">' . get_the_time( __( 'Y' , 'customizr-pro' ) ) . '</a>';
					else
						$trail[] = get_the_time( __( 'Y' , 'customizr-pro' ) );
				}
			}
		}
		...
		elseif ( is_search() ) {
			...
			if ( is_paged() )
				$trail[] = '<a href="' . get_search_link() . '" title="' . sprintf( esc_attr__( 'Search results for &quot;%1$s&quot;' , 'customizr-pro' ), esc_attr( get_search_query() ) ) . '">' . sprintf( __( 'Search results for &quot;%1$s&quot;' , 'customizr-pro' ), esc_attr( get_search_query() ) ) . '</a>';
			else
				$trail[] = sprintf( __( 'Search results for &quot;%1$s&quot;' , 'customizr-pro' ), esc_attr( get_search_query() ) );
		}

		/* If viewing a 404 error page. */
		elseif ( is_404() ) {
			$trail[] = __( '404 Not Found' , 'customizr-pro' );
		}

		/* Check for pagination. */
		if ( $maybe_paged ) {
			if ( is_paged() )
				$trail[] = sprintf( __( 'Page %d' , 'customizr-pro' ), absint( get_query_var( 'paged' ) ) );
			elseif ( is_singular() && 1 < get_query_var( 'page' ) )
				$trail[] = sprintf( __( 'Page %d' , 'customizr-pro' ), absint( get_query_var( 'page' ) ) );
		}
	...
	function czr_fn_breadcrumb_trail_get_bbpress_items( $args = array() ) {
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() )
				$trail[] = __( 'Split' , 'customizr-pro' );

			/* If viewing a topic merge page. */
			elseif ( bbp_is_topic_merge() )
				$trail[] = __( 'Merge' , 'customizr-pro' );
	...
	function czr_fn_breadcrumb_trail_map_rewrite_tags( $post_id = '' , $path = '' , $args = array() ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' , $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'Y' , 'customizr-pro' ), $post_id ) . '">' . get_the_time( __( 'Y' , 'customizr-pro' ), $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' , $post_id ), get_the_time( 'm' , $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y' , 'customizr-pro' ), $post_id ) . '">' . get_the_time( __( 'F' , 'customizr-pro' ), $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$trail[] = '<a href="' . get_day_link( get_the_time( 'Y' , $post_id ), get_the_time( 'm' , $post_id ), get_the_time( 'd' , $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y' , 'customizr-pro' ), $post_id ) . '">' . get_the_time( __( 'd' , 'customizr-pro' ), $post_id ) . '</a>';
       ...
       */
        function czr_fn_comment_title() {
          if ( 1 == get_comments_number() ) {
            $_title = __( 'One thought on', 'customizr-pro' );
          } else {
            $_title = sprintf( '%1$s %2$s', number_format_i18n( get_comments_number(), 'customizr' ) , __( 'thoughts on', 'customizr-pro' ) );
          }
       ...
       function czr_fn_comment_callback( $comment, $args, $depth ) {
        ...
        <li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
          <article id="comment-<?php comment_ID(); ?>" class="comment">
            <p><?php _e( 'Pingback:' , 'customizr-pro' ); ?> <?php comment_author_link(); ?>
                <?php if ( ! czr_fn_is_customizing() )  edit_comment_link( __( '(Edit)' , 'customizr-pro' ), '<span class="edit-link btn btn-success btn-mini">' , '</span>' ); ?>
            </p>
          </article>
        <?php
            break;
              ...
              printf('<article id="comment-%9$s" class="comment"><div class="%1$s"><div class="%2$s">%3$s</div><div class="%4$s">%5$s %6$s %7$s %8$s</div></div></article>',
                  ...
                  $tc_show_comment_content ? sprintf('<div class="%1$s">%2$s</div>',
                                            ...
                                            get_comment_reply_link( array_merge(
                                                                        $args,
                                                                        array(  'reply_text' => __( 'Reply' , 'customizr-pro' ).' <span>&darr;</span>',
                                                                                'depth' => $depth,
                  ...
                  sprintf('<header class="comment-meta comment-author vcard">%1$s %2$s</header>',
                        sprintf( '<cite class="fn">%1$s %2$s %3$s</cite>' ,
                            ...
                            ( $comment->user_id === $post->post_author ) ? '<span> ' . __( 'Post author' , 'customizr-pro' ) . '</span>' : '' ,
                            ! czr_fn_is_customizing() && current_user_can( 'edit_comment', $comment->comment_ID ) ? '<p class="edit-link btn btn-success btn-mini"><a class="comment-edit-link" href="' . get_edit_comment_link( $comment->comment_ID ) . '">' . __( 'Edit' , 'customizr-pro' ) . '</a></p>' : ''
                        ),
                        sprintf( '<a class="comment-date" href="%1$s"><time datetime="%2$s">%3$s</time></a>' ,
                            esc_url( get_comment_link( $comment->comment_ID ) ),
                            get_comment_time( 'c' ),
                            /* translators: 1: date, 2: time */
                            sprintf( __( '%1$s at %2$s' , 'customizr-pro' ), get_comment_date(), get_comment_time() )
                        )
                  ),

                  ( '0' == $comment->comment_approved ) ? sprintf('<p class="comment-awaiting-moderation">%1$s</p>',
                    __( 'Your comment is awaiting moderation.' , 'customizr-pro' )
                    ) : '',
   ...
   */
    function czr_fn_comment_navigation () {
      if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through
        ...
        <nav id="comment-nav-below" class="navigation">
          <h3 class="assistive-text section-heading"><?php _e( 'Comment navigation' , 'customizr-pro' ); ?></h3>
          <ul class="pager">

            <?php if(get_previous_comments_link() != null) : ?>

              <li class="previous">
                <span class="nav-previous"><?php previous_comments_link( __( '<span class="meta-nav">&larr;</span> Older Comments' , 'customizr-pro' ) ); ?></span>
              </li>
            ...
            <?php if(get_next_comments_link() != null) : ?>
              ...
              <li class="next">
                <span class="nav-next"><?php next_comments_link( __( 'Newer Comments <span class="meta-nav">&rarr;</span>' , 'customizr-pro' ) ); ?></span>
              </li>
      ...
      if ( ! comments_open() && get_comments_number() ) :
        echo apply_filters( 'tc_comment_close' ,
          sprintf('<p class="nocomments">%1$s</p>',
            __( 'Comments are closed.' , 'customizr-pro' )
          )
        );
    ...
    */
    function czr_fn_set_comment_title($_defaults) {
      $_defaults['title_reply'] =  __( 'Leave a comment' , 'customizr-pro' );
      return $_defaults;
    }
    ...
    */
    function czr_fn_display_comment_bubble( $_title = null ) {
      ...
      return sprintf('%1$s <span class="comments-link"><a href="%2$s%3$s" title="%4$s %5$s" data-disqus-identifier="javascript:this.page.identifier">%6$s</a></span>',
        ...
        sprintf( '%1$s %2$s' , get_comments_number(), __( 'Comment(s) on' , 'customizr-pro' ) ),
    ...
    */
    function czr_fn_custom_bubble_comment( $_html , $_opt ) {
      return sprintf('%4$s<span class="tc-comment-bubble %1$s">%2$s %3$s</span>',
        ...
        get_comments_number(),
        'default' == $_opt ? '' : sprintf( _n( 'comment' , 'comments' , get_comments_number(), 'customizr-pro' ),
        ...
        if ( null == czr_fn_opt( 'tc_featured_page_'.$fp_single_id ) || ! czr_fn_opt( 'tc_featured_page_'.$fp_single_id ) ) {
            ...
            if ( ! czr_fn_is_customizing() && is_user_logged_in() && current_user_can('edit_theme_options') ) {
              $customizr_link              = sprintf( '<a href="%1$s" title="%2$s">%3$s</a>',
                ...
                __( 'Customizer screen' , 'customizr-pro' ),
                __( 'Edit now.' , 'customizr-pro' )
              );
            }
            ...
            $featured_page_title            =  apply_filters( 'tc_fp_title', __( 'Featured page' , 'customizr-pro' ), $fp_single_id, $featured_page_id);
            $text                           =  apply_filters(
                                                'tc_fp_text',
                                                sprintf( '%1$s %2$s',
                                                  __( 'Featured page description text : use the page excerpt or set your own custom text in the customizer screen.' , 'customizr-pro' ),
                                                  $customizr_link
                                                ),
                                                $fp_single_id,
                                                $featured_page_id
        ...
        if ( $tc_gallery_fancybox == 1 && $permalink == false ) //add the filter only if link to the attachment file/image
          {
              ...
              if ( empty( $_post ) || ( 'attachment' != $_post->post_type ) || ! $url = wp_get_attachment_url( $_post->ID ) )
                return __( 'Missing Attachment' , 'customizr-pro');
      ...
      */
      function czr_fn_content_heading_title( $_title ) {
        ...
        if ( is_singular() || ! apply_filters('tc_display_link_for_post_titles' , true ) )
          return is_null($_title) ? apply_filters( 'tc_no_title_post', __( '{no title} Read the post &raquo;' , 'customizr-pro' ) )  : $_title;
        else
          return sprintf('<a href="%1$s" rel="bookmark">%2$s</a>',
            get_permalink(),
            is_null($_title) ? apply_filters( 'tc_no_title_post', __( '{no title} Read the post &raquo;' , 'customizr-pro' ) )  : $_title
          );//end sprintf
      }
      ...
      function czr_fn_archive_title_and_class_callback( $_title = null, $_return_class = false ) {
        ...
        //404
          ...
          $content        = sprintf('<h1 class="entry-title %1$s">%2$s</h1>',
                ...
                apply_filters( 'tc_404_title' , __( 'Ooops, page not found' , 'customizr-pro' ) )
           );
        ...
        //search results
          ...
          $content        = sprintf( '<div class="row-fluid"><div class="%1$s"><h1 class="%2$s">%3$s%4$s %5$s </h1></div><div class="%6$s">%7$s</div></div>',
                ...
                have_posts() ? '' :  __( 'No' , 'customizr-pro' ).'&nbsp;' ,
                apply_filters( 'tc_search_results_title' , __( 'Search Results for :' , 'customizr-pro' ) ),
        ...
        // all archives
            ...
            if ( apply_filters ( 'tc_show_author_meta' , get_the_author_meta( 'description', $user_id  ) ) ) {
              $content    .= sprintf('%1$s<div class="author-info"><div class="%2$s">%3$s</div></div>',
                  ...
                  sprintf('<div class="%1$s">%2$s</div><div class="%3$s"><h2>%4$s</h2><div>%5$s</div></div>',
                      ...
                      sprintf( __( 'About %s' , 'customizr-pro' ), get_the_author() ),
          ...
          //time archives
            ...
            $archive_type   = is_day() ? sprintf( __( 'Daily Archives: %s' , 'customizr-pro' ), '<span>' . get_the_date() . '</span>' ) : __( 'Archives' , 'customizr-pro' );
            $archive_type   = is_month() ? sprintf( __( 'Monthly Archives: %s' , 'customizr-pro' ), '<span>' . get_the_date( _x( 'F Y' , 'monthly archives date format' , 'customizr-pro' ) ) . '</span>' ) : $archive_type;
            $archive_type   = is_year() ? sprintf( __( 'Yearly Archives: %s' , 'customizr-pro' ), '<span>' . get_the_date( _x( 'Y' , 'yearly archives date format' , 'customizr-pro' ) ) . '</span>' ) : $archive_type;
  ...
  class CZR_no_results {
      ...
      function czr_fn_no_result_content() {
          ...
          echo apply_filters( 'tc_no_result_content',
              sprintf('<div class="%1$s"><div class="entry-content"><p>%2$s</p> %3$s</div>%4$s</div>',
                  'tc-content span12',
                  __( 'Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'customizr-pro' ),
                  get_search_form( $echo = false ),
                  '<hr class="featurette-divider '.current_filter().'">'
              )//end sprintf
          );//end filter
    ...
    function czr_fn_page_content() {
      ...
      ob_start();
        ...
        <div class="entry-content">
          <?php
            the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>' , 'customizr-pro' ) );
            wp_link_pages( array(
                'before'        => '<div class="btn-toolbar page-links"><div class="btn-group">' . __( 'Pages:' , 'customizr-pro' ),
                'after'         => '</div></div>',
      ...
      do_action( '__before_content' );
        ?>
          <section class="<?php echo implode( ' ', apply_filters( 'tc_single_post_section_class', array( 'entry-content' ) ) ); ?> <?php echo $icon_class ?>">
              <?php the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>' , 'customizr-pro' ) ); ?>
              <?php wp_link_pages( array( 'before' => '<div class="pagination pagination-centered">' . __( 'Pages:' , 'customizr-pro' ), 'after' => '</div>' ) ); ?>
      ...
      foreach ( $authors_id as $author_id ) {
        ...
        $html         .= sprintf('<div class="author-info"><div class="%1$s">%2$s %3$s</div></div>',
                            ...
                            sprintf('<div class="%1$s"><h3>%2$s</h3><div>%3$s</div><div class="author-link">%4$s</div></div>',
                                    apply_filters( 'tc_author_meta_content_class', 'author-description span10' ),
                                    sprintf( __( 'About %s' , 'customizr-pro' ), $author_name ),
                                    apply_filters( 'the_author_description', get_the_author_meta( 'description', $author_id ) ),
                                    sprintf( '<a href="%1$s" rel="author">%2$s</a>',
                                      esc_url( get_author_posts_url( $author_id ) ),
                                      sprintf( __( 'View all posts by %s <span class="meta-nav">&rarr;</span>' , 'customizr-pro' ), $author_name )
                                    )
                            )
        );//end sprintf
      }//end for
  ...
  private function czr_fn_render_content_view( $_content_model ) {
    ...
    elseif ( in_array( get_post_format(), array( 'quote', 'status', 'link', 'aside', 'video' ) ) )
    {
      ...
      $_content   = sprintf( '%1$s%2$s',
        apply_filters( 'tc_the_content', get_the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>' , 'customizr-pro' ) ) ),
        wp_link_pages( array(
          'before'  => '<div class="pagination pagination-centered">' . __( 'Pages:' , 'customizr-pro' ),
          'after'   => '</div>',
          'echo'    => 0
          ) )
      );
    ...
    class CZR_post_metas {
        ...
        */
        private function czr_fn_build_post_post_metas_model() {
          ...
          */
          $_html      = wp_sprintf( __( 'This entry was posted on %1$s<span class="by-author"> by %2$s</span>.' , 'customizr-pro' ),
              $pub_date,
              $auth
          );
        ...
        */
        private function czr_fn_build_attachment_post_metas_model() {
          ...
          $_html = sprintf( '%1$s <span class="entry-date"><time class="entry-date updated" datetime="%2$s">%3$s</time></span> %4$s %5$s',
              '<span class="meta-prep meta-prep-entry-date">'.__('Published' , 'customizr-pro').'</span>',
              ...
              ( isset($metadata['width']) && isset($metadata['height']) ) ? __('at dimensions' , 'customizr-pro').'<a href="'.esc_url( wp_get_attachment_url() ).'" title="'.__('Link to full-size image' , 'customizr-pro').'"> '.$metadata['width'].' &times; '.$metadata['height'].'</a>' : '',
              __('in' , 'customizr-pro').'<a href="'.esc_url( get_permalink( $post->post_parent ) ).'" title="'.__('Return to ' , 'customizr-pro').esc_attr( strip_tags( get_the_title( $post->post_parent ) ) ).'" rel="gallery"> '.get_the_title( $post->post_parent ).'</a>.'
          );
        ...
        */
        function czr_fn_set_post_metas_elements( $_default , $_args = array() ) {
            ...
            if ( $_show_cats && $_show_tags )
                $_tax_text   .= __( 'This entry was posted in %1$s and tagged %2$s' , 'customizr-pro' );
            if ( $_show_cats && ! $_show_tags )
                $_tax_text   .= __( 'This entry was posted in %1$s' , 'customizr-pro' );
            if ( ! $_show_cats && $_show_tags )
                $_tax_text   .= __( 'This entry was tagged %2$s' , 'customizr-pro' );
            ...
            $_date_text = '';
            if ( $_show_pub_date ) {
              $_date_text        = empty($_tax_text) ? __( 'This entry was posted on %1$s' , 'customizr-pro' ) : $_date_text;
              if ( $_show_cats )
                $_date_text   .= __( 'on %1$s' , 'customizr-pro' );
              if ( ! $_show_cats && $_show_tags )
                $_date_text   .= __( 'and posted on %1$s' , 'customizr-pro' );
            ...
            if ( $_show_author ) {
              if ( empty($_tax_text) && empty($_date_text) ) {
                  $_author_text = sprintf( '%s <span class="by-author">%s</span>' , __( 'This entry was posted', 'customizr-pro' ), __('by %1$s' , 'customizr-pro') );
              } else {
                  $_author_text = sprintf( '<span class="by-author">%s</span>' , __('by %1$s' , 'customizr-pro') );
              }
            ...
            if ( $_show_upd_date ) {
              if ( $_show_upd_in_days ) {
                ...
                $_update_text = ( 0 == $_update_days ) ? __( '(updated today)' , 'customizr-pro' ) : sprintf( __( '(updated %s days ago)' , 'customizr-pro' ), $_update_days );
                $_update_text = ( 1 == $_update_days ) ? __( '(updated 1 day ago)' , 'customizr-pro' ) : $_update_text;
              }
              else {
                $_update_text = __( '(updated on %1$s)' , 'customizr-pro' );
              }
        ...
        */
        private function czr_fn_meta_term_view( $term ) {
          ...
          return apply_filters( 'tc_meta_term_view' , sprintf($_to_return,
              $_classes,
              $_term_link,
              esc_attr( sprintf( __( "View all posts in %s", 'customizr-pro' ), $term -> name ) ),
              $term -> name
            )
          );
        }
        ...
        private function czr_fn_get_meta_author() {
            ...
            foreach ( $author_id_array as $author_id ) {
                ...
                $_html      .= sprintf( '<span class="author vcard author_name"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>' ,
                      esc_url( get_author_posts_url( $author_id ) ),
                      esc_attr( sprintf( __( 'View all posts by %s' , 'customizr-pro' ), $author_name ) ),
                      $author_name
                );
            }
        ...
        function czr_fn_add_link_to_post_after_metas( $_metas_html ) {
          ...
          if ( apply_filters( 'tc_show_link_after_post_metas' , true )
            ...
            return apply_filters('tc_add_link_to_post_after_metas',
              sprintf('%1$s | <a href="%2$s" title="%3$s">%3$s &raquo;</a>', $_metas_html, get_permalink(), __('Open' , 'customizr-pro') )
            );
          }
          return $_metas_html;
        }
        ...
        */
        function czr_fn_set_tax_metas( $_html , $_tax = array() ) {
          ...
          extract($_tax);
          $cat_list = ! empty($cat_list) && $_show_cats ? sprintf( '&nbsp;%s %s' , __('in' , 'customizr-pro') , $cat_list ) : '';
          $tag_list = ! empty($tag_list) && $_show_tags ? sprintf( '&nbsp;%s %s' , __('tagged' , 'customizr-pro') , $tag_list ) : '';
        ...
        */
        function czr_fn_set_author_metas( $_html , $_auth = '' ) {
          ...
          return sprintf( '<span class="by-author"> %s %s</span>' , __('by' , 'customizr-pro'), $_auth );
        }
      ...
      function czr_fn_post_nav() {
        ...
        <nav id="<?php echo $html_id; ?>" class="<?php echo $post_nav_class; ?>">
              ...
              <h3 class="assistive-text">
                <?php echo apply_filters( 'tc_singular_nav_title', __( 'Post navigation' , 'customizr-pro' ) ) ; ?>
              </h3>
        ...
        <?php elseif ( $wp_query->max_num_pages > 1 && in_array($_context, array('archive', 'home') ) ) : ?>
          ...
          <nav id="<?php echo $html_id; ?>" class="<?php echo $post_nav_class; ?>">
            ...
            <h3 class="assistive-text">
              <?php echo apply_filters( 'tc_list_nav_title', __( 'Post navigation' , 'customizr-pro' ) ) ; ?>
            </h3>
                    ...
                    <span class="nav-previous">
                      <?php
                        $next_posts_link_args      = apply_filters(
                          ...
                          array(
                            'label'        => apply_filters( 'tc_list_nav_next_text', __( '<span class="meta-nav">&larr;</span> Older posts' , 'customizr-pro' ) ),
                            'max_pages'    => 0
                          )
                        );
                    ...
                    <span class="nav-next">
                      <?php
                        $previous_posts_link_args      = apply_filters(
                          ...
                          array(
                            'label'        => apply_filters( 'tc_list_nav_previous_text', __( 'Newer posts <span class="meta-nav">&rarr;</span>' , 'customizr-pro' ) ),
                            'max_pages'    => 0
                          )
                        );
...
class CZR_slider {
  ...
  */
  ...
  function czr_fn_render_slider_edit_link_view( $slides, $slider_name_id ) {
    ...
    $_text                   = sprintf( __( 'Customize or remove %s' , 'customizr-pro' ),
                                  ( 'tc_posts_slider' == $slider_name_id ) ?__('the posts slider', 'customizr-pro') : __('this slider', 'customizr-pro' )
                              );
  ...
  function czr_fn_set_demo_slide_data( $data, $slider_name_id, $id ) {
    ...
    switch ( $id ) {
      case 1 :
        ...
        $data['button_text']  = __( 'Discover the Customizr WordPress theme &raquo;' , 'customizr-pro');
      break;

      case 2 :
        $data['title']        = __( 'Discover how to replace or remove this demo slider.', 'customizr-pro' );
        $data['link_url']     = esc_url( 'docs.presscustomizr.com/article/102-customizr-theme-options-front-page#front-page-slider' );
        $data['button_text']  = __( 'Check the slider doc now &raquo;' , 'customizr-pro');
      break;
    };
  ...
  */
  ...
  function czr_fn_get_post_title( $_post, $default_title_length, $more ) {
    ...
    if ( ! empty( $_post->post_password ) ) {
      $protected_title_format = apply_filters( 'protected_title_format', __( 'Protected: %s', 'customizr-pro' ), $_post);
  ...
  */
  ...
  function czr_fn_get_post_excerpt( $_post, $default_text_length, $more ) {
    if ( ! empty( $_post->post_password) )
      return __( 'There is no excerpt because this is a protected post.', 'customizr-pro' );
  		    		...
  		    		sprintf( '<p>%1$s %2$s %3$s</p>',
                              ...
                              apply_filters( 'tc_credit_link',
                                  sprintf( '<span class="tc-credits-text">%1$s </span> &middot;',
                                      sprintf( __('Designed with the %s', 'customizr-pro'), sprintf( '<a class="czr-designer-link" href="%1$s" title="%2$s">%2$s</a>', esc_url( CZR_WEBSITE . 'customizr' ), __('Customizr theme', 'customizr-pro') ) )
                                  )
                              )
  					   )
	    		)
