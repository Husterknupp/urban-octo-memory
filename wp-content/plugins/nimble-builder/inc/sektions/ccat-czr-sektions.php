...
function nimble_add_i18n_localized_control_params( $params ) {
    return array_merge( $params, array(
        'i18n' => array(
            ...
            'The header location only accepts modules and pre-built header sections' => __('The header location only accepts modules and pre-built header sections', 'nimble-builder'),
            'The footer location only accepts modules and pre-built footer sections' => __('The footer location only accepts modules and pre-built footer sections', 'nimble-builder'),
          ...
          <?php
            ...
            foreach( $content_collection as $_params ) {
                ...
                printf('<div draggable="%7$s" data-sek-content-type="%1$s" data-sek-content-id="%2$s" title="%5$s"><div class="sek-module-icon %6$s">%3$s</div><div class="sek-module-title"><div class="sek-centered-module-title">%4$s</div></div></div>',
                      ...
                      $icon_img_html,
                      $_params['title'],
                      true === $_params['active'] ? __('Drag and drop or double-click to insert in your chosen target element.', 'nimble-builder' ) : __('Available soon ! This module is currently in beta, you can activate it in Site Wide Options > Beta features', 'nimble-builder'),
...
function sek_set_input_tmpl___reset_button( $input_id, $input_data ) {
    ?>
      ...
      <div class="sek-button-choice-wrapper">
        <input data-czrtype="<?php echo $input_id; ?>" type="hidden"/>
        <button type="button" aria-pressed="false" class="sek-ui-button sek-float-right" title="<?php _e('Reset', 'nimble-builder'); ?>" data-sek-reset-scope="<?php echo $input_data['scope']; ?>"><?php _e('Reset', 'nimble-builder'); ?></button>
      </div>
  <?php
}
?>
